...
let antinodes antenna_pairs w h = List.flatten (List.map (fun ((_, xa, ya), (_, xb, yb)) -> List.init (if w > h then w + 1 else h + 1) (fun i -> xa + i * (xa - xb), ya + i * (ya - yb))) antenna_pairs)
...
in print_int (List.fold_left (fun acc (x, y) -> if 0 <= x && x < width && 0 <= y && y < height then acc + 1 else acc) 0 (unique (antinodes (same_frequency_pairs (parse_map str_ls)) width height)))
